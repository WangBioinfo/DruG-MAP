"""
@File   :   model_x2
@Desc   :   Reconstruction model for Treatment profile data
"""
...
class Model_x2(torch.nn.Module):
    ...
    def _average_latent(self, x2):
        z, _, _ = self.encode(x2)
        mean_z = z.mean(0)
        return mean_z

    def forward(self, x2):
        # Forward pass through full network
        z, mu, logvar = self.encode(x2)
        rec_x2 = self.decode(z)
        return rec_x2, mu, logvar

    def forward_detach(self, x2):
        # Forward pass through full network
        z, mu, logvar = self.encode(x2)
        rec_x2 = self.decode(z.detach())
        return rec_x2, mu, logvar
    ...
    def train_model(self, learning_rate, weight_decay, n_epochs, train_loader, test_loader, save_model=True,
                    metrics_func=None, batch_size=64):
        ...
        # Loss item
        loss_item = ['loss', 'mse_x2', 'kld_x2', 'loss_D', 'loss_G']
        ...
        for epoch in range(n_epochs):
            ...
            loss_value = 0
            self.train()
            for i, (x2_train, CP_index, mol_id) in enumerate(train_loader):
                # 将x2数据送到运算设备
                x2_train = x2_train.to(self.dev)
                if x2_train.shape[0] == 1:
                    continue
                train_size += x2_train.shape[0]
                # Pass through full network
                x2_rec, mu2, logvar2 = self.forward(x2_train)

                # Update the generator (once every n_critic steps)
                ...
                loss, _, kld = self.loss(x2_rec, x2_train, mu2, logvar2)
            ...
            test_pearson = test_metrics_dict['x2_rec_pearson']
            ...
            print(
                f'[Epoch {epoch}] | loss: {train_loss:.3f}, mse_x2_rec: {train_mse_x2:.3f}, kld_x2: {train_kld_x2:.3f}, train_loss_D: {train_loss_D:.5f}, train_loss_G: {train_loss_G:.5f}| valid_loss: {test_loss:.3f}, valid_mse_x2_rec: {test_mse_x2:.3f}, valid_kld_x2: {test_kld_x2:.3f}, test_loss_D: {test_loss_D:.5f}, test_loss_G: {test_loss_G:.5f} | last_loss: {loss:.3f}, last_loss_all: {loss_vae_gan:.5f}, last_loss_G: {loss_gen:.5f}, last_loss_D: {loss_disc:.5f} |')
    ...
    def test_model(self, loader, loss_item=None, metrics_func=None):
        ...
        self.eval()
        with torch.no_grad():
            for x2_train, CP_index, mol_id in loader:
                x2_data = x2_train.to(self.dev)
                CP_index = np.array(list(CP_index))
                mol_id = np.array(list(mol_id))
                test_size += x2_data.shape[0]
                x2_rec, mu1, logvar1 = self.forward(x2_data)
                loss_ls = self.loss(x2_rec, x2_data, mu1, logvar1)

                outputs_real = D.forward(x2_data)
                outputs_fake = D.forward(x2_rec)
                ...
                if metrics_func != None:
                    metrics_dict, metrics_dict_ls = self.eval_x2_reconstruction(x2_data, x2_rec,
                                                                                metrics_func=metrics_func)
    ...
    def eval_x2_reconstruction(self, x2, x2_rec, metrics_func=['r2']):
        # Initial value
    ...
    def recon_profile(self, loader):
        ...
        with torch.no_grad():
            for x2_train, CP_index, mol_id in loader:
                ...
                x2_rec, mu2, logvar2 = self.forward(x2_data)
