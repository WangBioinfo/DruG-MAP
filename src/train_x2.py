"""
@File   :   train_x2
@Desc   :   Pretrain x2 model
"""

from dataset import Dataset_x2
from model_x2 import Model_x2
from utils import *
import argparse
import warnings
...
def train_x2(args):
    ...
    # Load data
    """
    dict_keys(['CP_index', 'mol_id', 'train_feat', 'x2'])
    """
    ...
    # Out dir
    local_out = '../results/pretrain_model_x2/'
    ...
    train = Dataset_x2(pair)
    valid = Dataset_x2(pairv)
    test = Dataset_x2(pairt)
    ...
    if train_flag:
        model = Model_x2(n_features=737, n_latent=n_latent, n_en_hidden=en_hidden, n_de_hidden=de_hidden,
        ...
        epoch_hist, best_epoch = model.train_model(train_loader=train_loader, test_loader=valid_loader,
                                                   ...
                                                   weight_decay=weight_decay, save_model=True, metrics_func=['pearson'])

        epoch_result = pd.DataFrame.from_dict(epoch_hist)
        epoch_result['epoch'] = np.arange(n_epochs)
        # Save loss value for each epoch
        epoch_result.to_csv(local_out + f'loss_epoch{n_epochs}.csv', index=False)
    ...
    if eval_metric:
        ...
        for name, rec_dict_value in zip(['test'], [test_metrics_dict_ls]):
            ...
            df_rec.to_csv(save_dir + '/{}_restruction_eval_x2_gan_epoch{}.csv'.format(name, n_epochs), index=False)
        ddict_data_test = model.recon_profile(loader=test_loader)
        save_HDF5(save_dir + '/test_restruction_profile.h5', ddict_data_test)
        print('Evaluation of the pretrain dataset x2 is complete.')
